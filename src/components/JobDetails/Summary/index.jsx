...
const Summary = () => {
	...
	useEffect(() => {
		...
		let balance = jobTotal - firstPayment - deductable - upgrades - finalPayment
	...
	}, [fields.job_total, fields.first_payment, fields.deductable, fields.upgrades, fields.final_payment])
	...
	const handleChange = (e) => {
		...
		const balance = jobTotal - firstPayment - deductable - upgrades - finalPayment
	...
	const getSummaryFields = async () => {
		...
		try {
			setLoading(true)
			const response = await clientBaseURL.get(`${clientEndPoints?.getJobSummary}/${id}`, {
				headers: {
	...
	const handleSubmit = async (e) => {
		try {
			...
			const response = await clientBaseURL.post(`${clientEndPoints?.updateJobSummary}/${id}`, fields, {
				headers: {
					Authorization: `Bearer ${token}`,
				},
			})
	...
	return (
		<div className="space-y-6">
			...
			<Form onSubmit={handleSubmit} className="bg-stone-200 rounded-2xl p-4">
				...
				<div className="flex flex-col lg:flex-row justify-between w-full max-w-screen-xl mb-4">
				...
				<div className="flex items-center justify-end mb-4">
					<Button type="submit" variant="gradient" className="text-sm" disabled={isSubmitting}>
						{isSubmitting ? (
