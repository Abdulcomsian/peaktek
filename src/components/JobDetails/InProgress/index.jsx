...
import { createQCInspection, getQCInspection as getQCInspectionApi, updateIPstatus } from '@services/apiInProgress'
				...
				<form onSubmit={handleSubmit(onSubmit)} className="mb-4">
					...
					<div className="flex items-center gap-3 mt-3">
						<CustomDatePicker
							...
							error={errors.date && formateErrorName(errors?.build_start_date?.message)}
						/>
						<CustomDatePicker
							...
							control={control}
							name="build_end_date"
							error={errors.date && formateErrorName(errors?.build_end_date?.message)}
						/>
					</div>
					<CkeditorControlled control={control} name="notes" id="notes" />

					<Button type="submit" variant="gradient" disabled={isSubmitting} className="mt-4">
						{isSubmitting ? (
