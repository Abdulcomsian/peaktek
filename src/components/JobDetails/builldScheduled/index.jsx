...
const BuildScheduledTab = () => {
  ...
  useEffect(() => {
    const fetchBuildSchedule = async () => {
      ...
      if (resp.status >= 200 && resp.status < 300) {
        ...
        const formattedTime = dayjs(buildData.build_time, "h:mm A").format(
          "HH:mm"
        );
  ...
  const handleBuildConfirmation = async function (e) {
    ...
    try {
      ...
      console.log("adjustor meeting status resp", resp);
      if (resp.data[0].confirmed === "true") {
        dispatch(setActiveTab("in-progress"));
        navigate(`/job-details/${jobId}/in-progress`);
      }
