...
import { createReadyToBuild, getReadyToBuild, updateReadyToBuildStatus } from '@services/apiReadyToBuild'
...
const ReadyToBuild = () => {
	...
	const onSubmit = async function (data) {
		const { home_owner, home_owner_email, date, notes, attachements, status } = data
		console.log(data)
	...
	const handleStatusChange = async function (e) {
		...
		try {
			...
			if (resp.status >= 200 && resp.status < 300) {
				dispatch(setActiveTab('build-scheduled'))
				navigate(`/job-details/${jobId}/build-scheduled`)
			}
		} finally {
	...
	return (
		<form onSubmit={handleSubmit(onSubmit)}>
			<div className="flex items-center gap-3">
				...
				/>
				{isChangeStatus && <Loader width={'24px'} height={'24px'} color="#000" />}
			</div>
			<div className="bg-white p-5 rounded-2xl">
				...
				<CkeditorControlled control={control} name="notes" id="notes" className="mb-3" />
